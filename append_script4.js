...
const newCode = `    const userIds = new Set<string>();
    ...
    const userMap: Record<string, string> = (usersData || []).reduce((acc: Record<string, string>, u) => { acc[u.id] = u.name || 'Staff'; return acc }, {});
...
code = code.replace(/return \{\s+id: child\.id,.*?\n\s+vitals:.*?\n\s+screenings:.*?\n\s+flags:.*?\n\s+referrals:.*?\n\s+\}\n\}/, newCode);
fs.writeFileSync('src/lib/dpo/actions.ts', code);
